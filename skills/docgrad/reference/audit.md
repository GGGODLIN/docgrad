...
3. **Check `inventory.untracked.count` before you write the rating down** — the corpus is collected off the filesystem, not out of git, so this rating can depend on whose checkout it was run in:
   ...
   - **`null`** → the check could not run: either git is not installed or this directory is not a git working tree. **`untracked.note` names which**, and the two need different follow-ups — install git or run somewhere else, versus the check can never apply here, so stop recommending it. Pass the note through rather than paraphrasing it back into "git was unavailable". State that in the report; `null` is not zero, and an unrun check must not be reported as a clean one.
