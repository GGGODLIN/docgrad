...
2. Read it and follow it exactly: pick the command the user asked for (`init`, `audit`, `improve`, `loop`, `report`), and load only the `reference/` files that `SKILL.md` names for that command.
...
4. If the user passed no argument, run `audit` when `.docgrad.yml` exists and `init` when it does not.
